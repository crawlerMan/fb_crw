...
def save_to_file(name, elements, status, current_section,username):
    ...
    try:
        ...
        if status == 0:
            ...
            try:
                for i in results:
                    i2 = db.profile_list.insert_one({"profile_id": i, "crawlStatus": False})
    ...
    try:
        ...
        try:
            ...
            if platform_ in ['linux', 'darwin']:
                driver = webdriver.Chrome(executable_path="/Users/vahid/Downloads/Ultimate-Facebook-Scraper-master/Code/chromedriver", options=options)
            else:
